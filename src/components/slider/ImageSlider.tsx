...
const ImageSlider: React.FC<
  {
    images: CommonCardSectionProps["images"];
  } & { title?: string }
> = ({ images, title }) => {
  ...
  return (
    <div className="w-full aspect-[4/1.37] relative">
      ...
      />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 max-w-[57rem] w-full flex items-center justify-between">
        <button
          className={`w-12 aspect-square rounded-full bg-background-1 flex items-center justify-center rotate-180 ${buttonPrevClassName}`}
        >
          <ButtonNext />
        </button>
        <button
          className={`w-12 aspect-square rounded-full bg-background-1 flex items-center justify-center ${buttonNextClassName}`}
        >
          <ButtonNext />
        </button>
      </div>
